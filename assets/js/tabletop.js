    ...
    const analyseBattlemapCartography = async () => {
        ...
        const livingContourCandidates = (options = {}) => {
            ...
            const publishContourEvidenceAudit = (emittedSuggestions) => {
                // IV.30.1G.5Z.38 — Independently inspect directional ink runs on
                // the ORIGINAL darkness mesh, not recovered floor or frontier edges.
                ...
                const independentWallCoverage = (() => {
                    ...
                    const collect = (path) => {
                        const points = Array.isArray(path?.points) ? path.points : [];
                        for (let i = 1; i < points.length; i += 1) represented.push([points[i - 1], points[i]]);
                    };
                    authoritativeContourSuggestions.forEach(collect);
                    promotedReconstructedSurfaceSuggestions.forEach(collect);
                    const nearExisting = (x, y) => represented.some(([a, b]) => {
                        const dx = b.x - a.x, dy = b.y - a.y;
                        const len = dx * dx + dy * dy;
                        const t = len ? Math.max(0, Math.min(1, ((x-a.x)*dx+(y-a.y)*dy)/len)) : 0;
                        return Math.hypot(x-a.x-t*dx, y-a.y-t*dy) <= contourStep * 1.5;
                    });
                    for (const [axis, dx, dy] of [['horizontal', 1, 0], ['vertical', 0, 1]]) {
                        for (let y = 2; y < contourRows - 2; y += 1) {
                            for (let x = 2; x < contourColumns - 2; x += 1) {
                                if (Number(darkness[y]?.[x] ?? 0) < threshold ||
                                    Number(darkness[y-dy]?.[x-dx] ?? 0) >= threshold) continue;
                                let length = 0, near = 0;
                                while (x + length*dx < contourColumns-2 && y + length*dy < contourRows-2 &&
                                    Number(darkness[y+length*dy]?.[x+length*dx] ?? 0) >= threshold) {
                                    const gx = (x+length*dx+.5)*contourStep;
                                    const gy = (y+length*dy+.5)*contourStep;
                                    if (nearExisting(gx, gy)) near += 1;
                                    length += 1;
                                }
                                if (length < minimumRun) continue;
                                runs.push({ axis, start: [x*contourStep, y*contourStep],
                                    end: [(x+(length-1)*dx)*contourStep, (y+(length-1)*dy)*contourStep],
                                    samples: length, nearRepresented: near,
                                    classification: near === length ? 'near-represented-ink-run' :
                                        near ? 'mixed-ink-run-review' : 'unrepresented-ink-run-review',
                                    wallCertified: false });
                            }
                        }
                    }
                    const review = runs.filter((run) => run.nearRepresented < run.samples);
                    return { diagnosticOnly: true, scope: 'independent-original-ink-directional-run-screen-not-wall-certification',
                        sourcePresent: Boolean(mesh && Array.isArray(darkness)),
                        threshold, minimumRun, meshCellsExamined: Math.max(0, contourRows-4)*Math.max(0, contourColumns-4),
                        runs, reviewRuns: review.length,
                        reviewSamples: review.reduce((sum, run) => sum + run.samples-run.nearRepresented, 0),
                        reviewBounds: review.slice(0, 12).map((run) => `${run.axis}@(${run.start.join(',')})-(${run.end.join(',')})/${run.samples}samples/${run.nearRepresented}near`),
                        limitation: 'directional-ink-can-be-grid-hatching-decoration-or-wall;not-exhaustive-illustration-coverage',
                        wholeIllustrationCoverageCertified: false, missingWallsCertified: false,
                        wallCertified: false, admittedEdges: 0, restoredRuns: 0 };
                })();
                // IV.30.1G.5Z.39 — Reconcile the independent directional review
                // against local ink and nearby represented geometry. Diagnostic only.
                // A matching ink component is not independent wall corroboration.
                const directionalInkContext = (() => {
                    const reviewRuns = independentWallCoverage.runs.filter((run) =>
                        run.nearRepresented < run.samples);
                    const localComponents = independentIllustratedWallSurvey.localInkFeatureClassification || [];
                    const inkAt = (x, y) => Number(darkness[y]?.[x] ?? 0);
                    const reviews = reviewRuns.map((run) => {
                        const vertical = run.axis === 'vertical';
                        const x0 = Math.round(run.start[0] / contourStep);
                        const y0 = Math.round(run.start[1] / contourStep);
                        const samples = [];
                        let parallel = 0, perpendicular = 0, branch = 0;
                        for (let i = 0; i < run.samples; i += 1) {
                            const x = x0 + (vertical ? 0 : i);
                            const y = y0 + (vertical ? i : 0);
                            const px = inkAt(x - (vertical ? 1 : 0), y - (vertical ? 0 : 1)) >= independentWallCoverage.threshold;
                            const nx = inkAt(x + (vertical ? 1 : 0), y + (vertical ? 0 : 1)) >= independentWallCoverage.threshold;
                            if (px || nx) perpendicular += 1;
                            if (px && nx) branch += 1;
                            if (inkAt(x - (vertical ? 0 : 1), y - (vertical ? 1 : 0)) >= independentWallCoverage.threshold ||
                                inkAt(x + (vertical ? 0 : 1), y + (vertical ? 1 : 0)) >= independentWallCoverage.threshold) parallel += 1;
                            samples.push([(x + .5) * contourStep, (y + .5) * contourStep]);
                        }
                        // The earlier local review uses mesh-centre coordinates.
                        // Compare actual sample coordinates, not the run's cell corners.
                        const overlaps = localComponents.map((component, index) => {
                            const bounds = component.bounds || component.boundingBox || null;
                            const points = component.samples || component.samplePoints || [];
                            const matching = samples.filter(([x, y]) =>
                                (Array.isArray(points) && points.some((point) =>
                                    Math.abs((Array.isArray(point) ? point[0] : point.x) - x) < contourStep * .51 &&
                                    Math.abs((Array.isArray(point) ? point[1] : point.y) - y) < contourStep * .51)) ||
                                (Array.isArray(bounds) && bounds.length === 4 &&
                                    x >= bounds[0] - contourStep && x <= bounds[2] + contourStep &&
                                    y >= bounds[1] - contourStep && y <= bounds[3] + contourStep)).length;
                            return matching ? { localComponent: index + 1, matchingSamples: matching } : null;
                        }).filter(Boolean);
                        return { axis: run.axis, start: run.start, end: run.end,
                            samples: run.samples, nearRepresented: run.nearRepresented,
                            parallelInkSamples: parallel, perpendicularInkSamples: perpendicular,
                            branchingSamples: branch, localReviewOverlaps: overlaps,
                            classification: 'directional-ink-context-unverified-not-wall-certified',
                            wallCertified: false };
                    });
                    return { diagnosticOnly: true, scope: 'directional-original-ink-local-context-not-wall-certification',
                        sourcePresent: Boolean(independentWallCoverage.sourcePresent && Array.isArray(localComponents)),
                        reviewRuns: reviews.length, reviews,
                        limitation: 'local-ink-overlap-and-directional-continuity-do-not-prove-structural-wall-identity',
                        wholeIllustrationCoverageCertified: false, missingWallsCertified: false,
                        wallCertified: false, admittedEdges: 0, restoredRuns: 0 };
                })();
                // IV.30.1G.5Z.40 — Illustrated Boundary Correspondence Audit.
                // Sample original ink along existing represented segments at several
                // normal offsets. This measures grid alignment sensitivity, NOT
                // missing-wall identity. No candidate is emitted or persisted.
                const illustratedBoundaryCorrespondence = (() => {
                    const paths = authoritativeContourSuggestions.concat(promotedReconstructedSurfaceSuggestions);
                    const threshold = independentWallCoverage.threshold;
                    const inkAtWorld = (x, y) => {
                        const col = Math.floor(x / contourStep);
                        const row = Math.floor(y / contourStep);
                        return Number(darkness[row]?.[col] ?? 0);
                    };
                    const offsets = [0, contourStep * .5, contourStep, contourStep * 1.5];
                    const totals = offsets.map((offset) => ({ offset, samples: 0, inkHits: 0 }));
                    let segments = 0, skippedSegments = 0, sampleBudget = 120000;
                    const lowCorrespondence = [];
                    for (const path of paths) {
                        ...
                        for (let index = 1; index < points.length; index += 1) {
                            ...
                            if (!Number.isFinite(length) || length < 1e-8 || sampleBudget <= 0) {
                                skippedSegments += 1; continue;
                            }
                            segments += 1;
                            const nx = -dy / length, ny = dx / length;
                            const count = Math.min(Math.max(1, Math.ceil(length / contourStep)), sampleBudget);
                            sampleBudget -= count;
                            let directHits = 0, offsetHits = 0;
                            for (let sample = 0; sample < count; sample += 1) {
                                const t = (sample + .5) / count;
                                const x = a.x + dx * t, y = a.y + dy * t;
                                offsets.forEach((offset, offsetIndex) => {
                                    const hit = offset === 0 ? inkAtWorld(x, y) >= threshold :
                                        inkAtWorld(x + nx * offset, y + ny * offset) >= threshold ||
                                        inkAtWorld(x - nx * offset, y - ny * offset) >= threshold;
                                    totals[offsetIndex].samples += 1;
                                    if (hit) totals[offsetIndex].inkHits += 1;
                                    if (offsetIndex === 0 && hit) directHits += 1;
                                    if (offsetIndex === 2 && hit) offsetHits += 1;
                                });
                            }
                            if (count >= 2 && directHits / count < .25 && lowCorrespondence.length < 12) {
                                lowCorrespondence.push({ pathIndex: paths.indexOf(path) + 1,
                                    segmentIndex: index, samples: count, directHits, offsetHits,
                                    classification: 'represented-segment-low-original-ink-review-not-missing-wall' });
                            }
                        }
                    }
                    return { diagnosticOnly: true,
                        scope: 'represented-boundary-original-ink-offset-sampling-not-exhaustive-coverage',
                        sourcePresent: Array.isArray(darkness) && contourStep > 0,
                        representedPaths: paths.length, representedSegments: segments,
                        skippedSegments, sampleBudgetExhausted: sampleBudget === 0,
                        offsets: totals, lowCorrespondence,
                        independentDirectionalReviewRuns: independentWallCoverage.reviewRuns,
                        limitation: 'offset-ink-may-be-hatching-or-decoration;low-ink-may-be-grid-alignment;unrepresented-walls-not-enumerated',
                        wholeIllustrationCoverageCertified: false, missingWallsCertified: false,
                        wallCertified: false, admittedEdges: 0, restoredRuns: 0 };
                })();
                // IV.30.1G.5Z.41 — Sampling Alignment & Structural Ink Discrimination Audit.
                // Paired normal samples are measured separately: a hit on either side
                // is not evidence that the illustrated line follows the wall centre.
                const samplingAlignmentAudit = (() => {
                    const paths = authoritativeContourSuggestions.concat(promotedReconstructedSurfaceSuggestions);
                    const threshold = independentWallCoverage.threshold;
                    const inkAt = (x, y) => Number(darkness[Math.floor(y / contourStep)]?.[Math.floor(x / contourStep)] ?? 0);
                    const offsets = [0, contourStep * .5, contourStep, contourStep * 1.5];
                    const totals = offsets.map((offset) => ({ offset, samples: 0, positive: 0, negative: 0,
                        paired: 0, positiveOnly: 0, negativeOnly: 0, neither: 0 }));
                    const reviews = [];
                    let segments = 0, budget = 120000, skipped = 0;
                    paths.forEach((path, pathIndex) => {
                        const points = Array.isArray(path?.points) ? path.points : [];
                        for (let i = 1; i < points.length; i += 1) {
                            const a = points[i - 1], b = points[i];
                            const dx = b.x - a.x, dy = b.y - a.y, length = Math.hypot(dx, dy);
                            if (!Number.isFinite(length) || length < 1e-8 || budget <= 0) { skipped++; continue; }
                            segments++;
                            const count = Math.min(Math.max(1, Math.ceil(length / contourStep)), budget);
                            budget -= count;
                            const nx = -dy / length, ny = dx / length;
                            const segment = offsets.map(() => ({ positive: 0, negative: 0, paired: 0, positiveOnly: 0, negativeOnly: 0, neither: 0 }));
                            for (let sample = 0; sample < count; sample++) {
                                const t = (sample + .5) / count, x = a.x + dx * t, y = a.y + dy * t;
                                offsets.forEach((offset, j) => {
                                    const positive = inkAt(x + nx * offset, y + ny * offset) >= threshold;
                                    const negative = inkAt(x - nx * offset, y - ny * offset) >= threshold;
                                    const total = totals[j], local = segment[j];
                                    total.samples++;
                                    if (positive) { total.positive++; local.positive++; }
                                    if (negative) { total.negative++; local.negative++; }
                                    const category = positive && negative ? 'paired' : positive ? 'positiveOnly' : negative ? 'negativeOnly' : 'neither';
                                    total[category]++; local[category]++;
                                });
                            }
                            if (count >= 2 && segment[0].positive / count < .25 && reviews.length < 12) {
                                const shifted = segment.slice(1);
                                const best = shifted.reduce((winner, entry, j) => {
                                    const hits = entry.positiveOnly + entry.negativeOnly + entry.paired;
                                    return hits > winner.hits ? { offset: offsets[j + 1], hits, ...entry } : winner;
                                }, { offset: 0, hits: 0 });
                                reviews.push({ pathIndex: pathIndex + 1, segmentIndex: i, samples: count,
                                    directHits: segment[0].positive, bestOffset: best.offset, bestHits: best.hits,
                                    positiveOnly: best.positiveOnly, negativeOnly: best.negativeOnly, paired: best.paired,
                                    classification: best.hits === 0 ? 'no-local-ink-correspondence' :
                                        best.paired > 0 ? 'offset-ink-both-sides-decoration-or-thick-line-unresolved' :
                                        'offset-ink-one-sided-alignment-or-decoration-unresolved' });
                            }
                        }
                    });
                    return { diagnosticOnly: true, scope: 'paired-normal-original-ink-sampling-not-structural-certification',
                        sourcePresent: Array.isArray(darkness) && contourStep > 0,
                        representedPaths: paths.length, representedSegments: segments, skippedSegments: skipped,
                        sampleBudgetExhausted: budget === 0, offsets: totals, lowInkReviews: reviews,
                        limitation: 'paired-normal-ink-is-not-a-wall-identity-test;decorative-hatching-and-grid-alignment-remain-unresolved;unrepresented-walls-not-enumerated',
                        wholeIllustrationCoverageCertified: false, missingWallsCertified: false,
                        wallCertified: false, admittedEdges: 0, restoredRuns: 0 };
                })();
                // IV.30.1G.5Z.42 — Source Image Alignment Audit.
                // Compare the exact source-to-analysis transform and independent pixel
                // readings against the existing coarse mesh; diagnostics never emit walls.
                const sourceImageAlignmentAudit = (() => {
                    const paths = authoritativeContourSuggestions.concat(promotedReconstructedSurfaceSuggestions);
                    const threshold = independentWallCoverage.threshold;
                    const sourceToCanvasX = canvas.width / image.naturalWidth;
                    const sourceToCanvasY = canvas.height / image.naturalHeight;
                    const boardToCanvasX = canvas.width / displayWidth;
                    const boardToCanvasY = canvas.height / displayHeight;
                    const validTransform = [sourceToCanvasX, sourceToCanvasY, boardToCanvasX, boardToCanvasY,
                        gridCanvasX, gridCanvasY, contourCellX, contourCellY].every((value) => Number.isFinite(value) && value > 0);
                    const sourcePixelAt = (x, y) => {
                        const px = originX + x * gridCanvasX;
                        const py = originY + y * gridCanvasY;
                        if (px < 0 || py < 0 || px >= canvas.width || py >= canvas.height) return null;
                        return luminance(px, py) <= darkThreshold;
                    };
                    const meshInkAt = (x, y) => {
                        const col = Math.floor(x / contourStep), row = Math.floor(y / contourStep);
                        return col >= 0 && row >= 0 && col < contourColumns && row < contourRows
                            ? darkness[row][col] >= threshold : null;
                    };
                    const offsets = [0, contourStep * .25, contourStep * .5, contourStep];
                    const totals = offsets.map((offset) => ({ offset, samples: 0, pixelHits: 0, meshHits: 0,
                        pixelOnly: 0, meshOnly: 0, both: 0, neither: 0, outsideImage: 0 }));
                    const reviews = [];
                    let segments = 0, remaining = 120000, skipped = 0;
                    paths.forEach((path, pathIndex) => {
                        const points = Array.isArray(path?.points) ? path.points : [];
                        for (let i = 1; i < points.length; i++) {
                            const a = points[i - 1], b = points[i];
                            const dx = b.x - a.x, dy = b.y - a.y, length = Math.hypot(dx, dy);
                            if (!Number.isFinite(length) || length < 1e-8 || remaining <= 0) { skipped++; continue; }
                            segments++;
                            const count = Math.min(Math.max(1, Math.ceil(length / contourStep)), remaining);
                            remaining -= count;
                            const nx = -dy / length, ny = dx / length;
                            let segmentPixelOnly = 0, segmentMeshOnly = 0;
                            for (let j = 0; j < count; j++) {
                                const t = (j + .5) / count, x = a.x + dx * t, y = a.y + dy * t;
                                offsets.forEach((offset, k) => {
                                    // Both normal directions use the same original-image pixel transform.
                                    const positions = offset === 0 ? [[x, y]] :
                                        [[x + nx * offset, y + ny * offset], [x - nx * offset, y - ny * offset]];
                                    positions.forEach(([sx, sy]) => {
                                        const pixel = sourcePixelAt(sx, sy), mesh = meshInkAt(sx, sy);
                                        const total = totals[k];
                                        if (pixel === null || mesh === null) { total.outsideImage++; return; }
                                        total.samples++;
                                        if (pixel) total.pixelHits++;
                                        if (mesh) total.meshHits++;
                                        const category = pixel && mesh ? 'both' : pixel ? 'pixelOnly' : mesh ? 'meshOnly' : 'neither';
                                        total[category]++;
                                        if (k === 0 && pixel && !mesh) segmentPixelOnly++;
                                        if (k === 0 && mesh && !pixel) segmentMeshOnly++;
                                    });
                                });
                            }
                            if ((segmentPixelOnly || segmentMeshOnly) && reviews.length < 12) {
                                reviews.push({ pathIndex: pathIndex + 1, segmentIndex: i,
                                    pixelOnly: segmentPixelOnly, meshOnly: segmentMeshOnly,
                                    classification: 'source-pixel-versus-mesh-disagreement-review-not-wall-certified' });
                            }
                        }
                    });
                    return { diagnosticOnly: true, scope: 'source-image-to-grid-transform-and-pixel-mesh-correspondence-not-wall-certification',
                        sourceWidth: image.naturalWidth, sourceHeight: image.naturalHeight,
                        analysisWidth: canvas.width, analysisHeight: canvas.height,
                        boardWidth: displayWidth, boardHeight: displayHeight,
                        sourceToCanvasX, sourceToCanvasY, boardToCanvasX, boardToCanvasY,
                        originX, originY, gridCanvasX, gridCanvasY, contourCellX, contourCellY,
                        contourColumns, contourRows, validTransform, representedPaths: paths.length,
                        representedSegments: segments, skippedSegments: skipped, sampleBudgetExhausted: remaining === 0,
                        offsets: totals, reviews,
                        limitation: 'single-pixel-versus-cell-density-is-not-a-wall-identity-test;image-resampling-and-hatching-remain-unresolved;unrepresented-walls-not-enumerated',
                        wholeIllustrationCoverageCertified: false, missingWallsCertified: false,
                        wallCertified: false, admittedEdges: 0, restoredRuns: 0 };
                })();
                // IV.30.1G.5Z.43 — Pixel-to-Mesh Fidelity Audit.
                // Independent local neighbourhood probes: diagnostics only; never feed wall promotion.
                const pixelMeshFidelityAudit = (() => {
                    const windows = [];
                    const radius = Math.max(1, Math.ceil(Math.max(contourCellX, contourCellY) / 2));
                    const threshold = independentWallCoverage.threshold;
                    const paths = authoritativeContourSuggestions.concat(promotedReconstructedSurfaceSuggestions);
                    let sampled = 0, pixelOnly = 0, meshOnly = 0, both = 0, neither = 0;
                    for (let pathIndex = 0; pathIndex < paths.length && windows.length < 12; pathIndex++) {
                        const points = paths[pathIndex]?.points || [];
                        for (let segmentIndex = 1; segmentIndex < points.length && windows.length < 12; segmentIndex++) {
                            const a = points[segmentIndex - 1], b = points[segmentIndex];
                            const x = (a.x + b.x) / 2, y = (a.y + b.y) / 2;
                            const px = originX + x * gridCanvasX, py = originY + y * gridCanvasY;
                            const col = Math.floor(x / contourStep), row = Math.floor(y / contourStep);
                            if (col < 0 || row < 0 || col >= contourColumns || row >= contourRows ||
                                px < radius || py < radius || px + radius >= canvas.width || py + radius >= canvas.height) continue;
                            const pixelDark = luminance(px, py) <= darkThreshold;
                            const meshDensity = darkness[row][col], meshDark = meshDensity >= threshold;
                            sampled++;
                            if (pixelDark && meshDark) both++;
                            else if (pixelDark) pixelOnly++;
                            else if (meshDark) meshOnly++;
                            else neither++;
                            if (pixelDark === meshDark) continue;
                            let neighbourhoodDark = 0, neighbourhoodSamples = 0;
                            for (let dy = -radius; dy <= radius; dy++) {
                                for (let dx = -radius; dx <= radius; dx++) {
                                    neighbourhoodSamples++;
                                    if (luminance(px + dx, py + dy) <= darkThreshold) neighbourhoodDark++;
                                }
                            }
                            windows.push({ pathIndex: pathIndex + 1, segmentIndex, gridX: x, gridY: y,
                                canvasX: px, canvasY: py, meshColumn: col, meshRow: row,
                                pixelDark, meshDark, meshDensity, threshold, radius,
                                neighbourhoodDark, neighbourhoodSamples,
                                classification: 'pixel-mesh-discrepancy-structural-identity-unverified' });
                        }
                    }
                    return { diagnosticOnly: true, scope: 'bounded-pixel-neighbourhood-versus-mesh-density-not-wall-certification',
                        sampled, pixelOnly, meshOnly, both, neither, windows,
                        limitation: 'neighbourhood-ink-may-be-grid-or-hatching;sampling-does-not-establish-wall-identity;whole-illustration-not-surveyed',
                        wallCertified: false, admittedEdges: 0, restoredRuns: 0 };
                })();
                // IV.30.1G.5Z.44 — Ink Preservation Audit.
                // Reproduce the exact inset/rounded mesh sampler, then compare with every
                // source pixel in the same cell. A dark centre pixel is not a dark cell.
                // All classifications are diagnostic and must never enter wall admission.
                const inkPreservationAudit = (() => {
                    const reviews = pixelMeshFidelityAudit.windows.map((window) => {
                        const col = window.meshColumn, row = window.meshRow;
                        const left = originX + col * contourCellX;
                        const top = originY + row * contourCellY;
                        const right = left + contourCellX, bottom = top + contourCellY;
                        const insetLeft = left + contourCellX * .14;
                        const insetTop = top + contourCellY * .14;
                        const insetRight = right - contourCellX * .14;
                        const insetBottom = bottom - contourCellY * .14;
                        const stepX = Math.max(1, Math.round(contourCellX / 5));
                        const stepY = Math.max(1, Math.round(contourCellY / 5));
                        let sampledDark = 0, sampledTotal = 0, fullDark = 0, fullTotal = 0;
                        let insetDark = 0, insetTotal = 0;
                        const xMin = Math.max(0, Math.ceil(left)), xMax = Math.min(canvas.width - 1, Math.floor(right));
                        const yMin = Math.max(0, Math.ceil(top)), yMax = Math.min(canvas.height - 1, Math.floor(bottom));
                        for (let y = yMin; y <= yMax; y++) {
                            for (let x = xMin; x <= xMax; x++) {
                                const dark = luminance(x, y) <= darkThreshold;
                                fullTotal++;
                                if (dark) fullDark++;
                                if (x >= insetLeft && x <= insetRight && y >= insetTop && y <= insetBottom) {
                                    insetTotal++;
                                    if (dark) insetDark++;
                                }
                            }
                        }
                        for (let y = Math.max(0, Math.round(insetTop)); y <= Math.min(canvas.height - 1, Math.round(insetBottom)); y += stepY) {
                            for (let x = Math.max(0, Math.round(insetLeft)); x <= Math.min(canvas.width - 1, Math.round(insetRight)); x += stepX) {
                                sampledTotal++;
                                if (luminance(x, y) <= darkThreshold) sampledDark++;
                            }
                        }
                        const reproducedDensity = sampledDark / Math.max(1, sampledTotal);
                        const samplerMatches = Math.abs(reproducedDensity - window.meshDensity) < 0.000001;
                        const centreInCell = window.canvasX >= left && window.canvasX < right && window.canvasY >= top && window.canvasY < bottom;
                        const centreInInset = window.canvasX >= insetLeft && window.canvasX <= insetRight && window.canvasY >= insetTop && window.canvasY <= insetBottom;
                        const fullDensity = fullDark / Math.max(1, fullTotal);
                        const insetDensity = insetDark / Math.max(1, insetTotal);
                        const reason = !centreInCell ? 'point-cell-coordinate-mismatch'
                            : !samplerMatches ? 'mesh-density-reproduction-mismatch'
                            : !centreInInset ? 'dark-point-outside-inset-sampler'
                            : sampledDark === 0 && insetDark > 0 ? 'inset-ink-missed-by-sparse-sampler'
                            : insetDark === 0 && fullDark > 0 ? 'ink-outside-inset-sampler'
                            : window.meshDensity < window.threshold ? 'cell-ink-below-density-threshold'
                            : 'pixel-versus-cell-classification-unresolved';
                        return { pathIndex: window.pathIndex, segmentIndex: window.segmentIndex,
                            meshColumn: col, meshRow: row, centreInCell, centreInInset,
                            sampledDark, sampledTotal, reproducedDensity, samplerMatches,
                            insetDark, insetTotal, insetDensity, fullDark, fullTotal, fullDensity,
                            reason, wallCertified: false };
                    });
                    return { diagnosticOnly: true, scope: 'bounded-original-pixel-versus-exact-mesh-sampler-not-wall-certification',
                        reviews, reproduced: reviews.filter((review) => review.samplerMatches).length,
                        mismatch: reviews.filter((review) => !review.samplerMatches).length,
                        limitation: 'pixel-preservation-is-not-structural-wall-evidence;bounded-reviews-not-exhaustive',
                        wallCertified: false, admittedEdges: 0, restoredRuns: 0 };
                })();
                // IV.30.1G.5Z.45 — Adaptive Ink Sampling Audit.
                // Compare bounded full-cell, inset and neighbouring-cell pixel coverage against
                // the unchanged production sampler. This is observational: no wall admission.
                const adaptiveInkSamplingAudit = (() => {
                    const reviews = inkPreservationAudit.reviews.map((review) => {
                        const col = review.meshColumn, row = review.meshRow;
                        const left = originX + col * contourCellX, top = originY + row * contourCellY;
                        const x0 = Math.max(0, Math.ceil(left)), y0 = Math.max(0, Math.ceil(top));
                        const x1 = Math.min(canvas.width - 1, Math.floor(left + contourCellX));
                        const y1 = Math.min(canvas.height - 1, Math.floor(top + contourCellY));
                        const insetX0 = left + contourCellX * .14, insetX1 = left + contourCellX * .86;
                        const insetY0 = top + contourCellY * .14, insetY1 = top + contourCellY * .86;
                        let fullDark = 0, fullTotal = 0, insetDark = 0, insetTotal = 0;
                        let horizontalPairs = 0, verticalPairs = 0, isolatedDark = 0;
                        const darkAt = (x, y) => x >= 0 && y >= 0 && x < canvas.width && y < canvas.height && luminance(x, y) <= darkThreshold;
                        for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
                            ...
                            if (!dark) continue;
                            if (darkAt(x + 1, y)) horizontalPairs++;
                            if (darkAt(x, y + 1)) verticalPairs++;
                            if (!darkAt(x - 1, y) && !darkAt(x + 1, y) && !darkAt(x, y - 1) && !darkAt(x, y + 1)) isolatedDark++;
                        }
                        // The adjacent mesh cells are compared without altering their density.
                        const neighbours = [[col - 1, row], [col + 1, row], [col, row - 1], [col, row + 1]]
                            .filter(([c, r]) => c >= 0 && r >= 0 && c < contourColumns && r < contourRows)
                            .map(([c, r]) => ({ column: c, row: r, density: darkness[r][c],
                                productionDark: darkness[r][c] >= independentWallCoverage.threshold }));
                        const fullDensity = fullDark / Math.max(1, fullTotal);
                        const insetDensity = insetDark / Math.max(1, insetTotal);
                        const threshold = independentWallCoverage.threshold;
                        const fullCellCandidate = fullDensity >= threshold;
                        const insetCandidate = insetDensity >= threshold;
                        const directionalContinuity = horizontalPairs > 0 || verticalPairs > 0;
                        return { pathIndex: review.pathIndex, segmentIndex: review.segmentIndex,
                            meshColumn: col, meshRow: row, productionDensity: review.reproducedDensity,
                            productionDark: review.reproducedDensity >= threshold, fullDark, fullTotal,
                            insetDark, insetTotal, fullDensity, insetDensity, fullCellCandidate, insetCandidate,
                            horizontalPairs, verticalPairs, isolatedDark, directionalContinuity,
                            neighbours, classification: !directionalContinuity ? 'isolated-or-disconnected-ink-review'
                                : fullCellCandidate || insetCandidate ? 'adaptive-density-candidate-not-wall-certified'
                                : 'thin-ink-below-adaptive-density-threshold-not-wall-certified', wallCertified: false };
                    });
                    return { diagnosticOnly: true, scope: 'bounded-adaptive-pixel-coverage-and-directional-neighbour-review-not-wall-certification',
                        reviews, fullCellCandidates: reviews.filter((r) => r.fullCellCandidate).length,
                        insetCandidates: reviews.filter((r) => r.insetCandidate).length,
                        directionalReviews: reviews.filter((r) => r.directionalContinuity).length,
                        limitation: 'adaptive-density-and-directional-pairs-may-be-grid-or-hatching;neighbouring-cell-density-does-not-prove-wall-continuity;bounded-reviews-not-exhaustive',
                        wallCertified: false, admittedEdges: 0, restoredRuns: 0 };
                })();
                ...
                const publishedEvidenceAudit = {
                    ...
                    independentWallCoverageAudit: independentWallCoverage,
                    directionalInkContextAudit: directionalInkContext,
                    illustratedBoundaryCorrespondenceAudit: illustratedBoundaryCorrespondence,
                    samplingAlignmentAudit: samplingAlignmentAudit,
                    sourceImageAlignmentAudit: sourceImageAlignmentAudit,
                    pixelMeshFidelityAudit: pixelMeshFidelityAudit,
        ...
        if (detail === 'audit') {
            ...
            if (cartographyEvidenceAudit && Array.isArray(endpointRecords)) {
                ...
                const independentWallCoverage = cartographyEvidenceAudit.independentWallCoverageAudit;
                const directionalContext = cartographyEvidenceAudit.directionalInkContextAudit;
                ...
                const adaptive = cartographyEvidenceAudit.adaptiveInkSamplingAudit;
                cartographyAuditRuntimeWitness.dataset.cartographyAdaptiveInkSampling = adaptive
                    ? `G.5Z.45 adaptive ink sampling · ${adaptive.scope} · ${adaptive.reviews.length} bounded reviews / ${adaptive.fullCellCandidates} full-cell candidates / ${adaptive.insetCandidates} inset candidates / ${adaptive.directionalReviews} directional reviews · [${adaptive.reviews.map((r) => `P${r.pathIndex}:S${r.segmentIndex}/cell(${r.meshColumn},${r.meshRow})/production${r.productionDensity.toFixed(3)}/full${r.fullDark}/${r.fullTotal}/inset${r.insetDark}/${r.insetTotal}/horizontal${r.horizontalPairs}/vertical${r.verticalPairs}/isolated${r.isolatedDark}/neighbours[${r.neighbours.map((n) => `${n.column},${n.row}:${n.density.toFixed(3)}`).join('|')}]/${r.classification}`).join(';')}] · ${adaptive.limitation} · wall certified false · 0 edges admitted · 0 runs restored`
                    : 'G.5Z.45 adaptive ink sampling unavailable';
                const preservation = cartographyEvidenceAudit.inkPreservationAudit;
                cartographyAuditRuntimeWitness.dataset.cartographyInkPreservation = preservation
                    ? `G.5Z.44 ink preservation · ${preservation.scope} · ${preservation.reviews.length} bounded reviews / ${preservation.reproduced} sampler matches / ${preservation.mismatch} sampler mismatches · [${preservation.reviews.map((r) => `P${r.pathIndex}:S${r.segmentIndex}/cell(${r.meshColumn},${r.meshRow})/centre-in-cell${Number(r.centreInCell)}/centre-in-inset${Number(r.centreInInset)}/sample${r.sampledDark}/${r.sampledTotal}/inset${r.insetDark}/${r.insetTotal}/full${r.fullDark}/${r.fullTotal}/${r.reason}`).join(';')}] · ${preservation.limitation} · wall certified false · 0 edges admitted · 0 runs restored`
                    : 'G.5Z.44 ink preservation unavailable';
                const fidelity = cartographyEvidenceAudit.pixelMeshFidelityAudit;
                cartographyAuditRuntimeWitness.dataset.cartographyPixelMeshFidelity = fidelity
                    ? `G.5Z.43 pixel-to-mesh fidelity · ${fidelity.scope} · ${fidelity.sampled} sampled / ${fidelity.pixelOnly} pixel-only / ${fidelity.meshOnly} mesh-only / ${fidelity.both} both / ${fidelity.neither} neither · ${fidelity.windows.length} discrepancy windows [${fidelity.windows.map((w) => `P${w.pathIndex}:S${w.segmentIndex}@(${w.gridX.toFixed(2)},${w.gridY.toFixed(2)})/pixel${Number(w.pixelDark)}/mesh${Number(w.meshDark)}/density${w.meshDensity.toFixed(3)}/neighbourhood${w.neighbourhoodDark}/${w.neighbourhoodSamples}`).join(';')}] · ${fidelity.limitation} · wall certified false · 0 edges admitted · 0 runs restored`
                    : 'G.5Z.43 pixel-to-mesh fidelity unavailable';
                const sourceAlignment = cartographyEvidenceAudit.sourceImageAlignmentAudit;
                cartographyAuditRuntimeWitness.dataset.cartographySourceImageAlignment = sourceAlignment
                    ? `G.5Z.42 source image alignment · ${sourceAlignment.scope} · source ${sourceAlignment.sourceWidth}x${sourceAlignment.sourceHeight} / analysis ${sourceAlignment.analysisWidth}x${sourceAlignment.analysisHeight} / board ${sourceAlignment.boardWidth}x${sourceAlignment.boardHeight} · ` +
                        `source-to-canvas (${sourceAlignment.sourceToCanvasX.toFixed(5)},${sourceAlignment.sourceToCanvasY.toFixed(5)}) / board-to-canvas (${sourceAlignment.boardToCanvasX.toFixed(5)},${sourceAlignment.boardToCanvasY.toFixed(5)}) · ` +
                        `origin (${sourceAlignment.originX.toFixed(2)},${sourceAlignment.originY.toFixed(2)}) / grid-cell (${sourceAlignment.gridCanvasX.toFixed(2)},${sourceAlignment.gridCanvasY.toFixed(2)}) / mesh-cell (${sourceAlignment.contourCellX.toFixed(2)},${sourceAlignment.contourCellY.toFixed(2)}) / mesh ${sourceAlignment.contourColumns}x${sourceAlignment.contourRows} · ` +
                        `transform valid ${sourceAlignment.validTransform} · ${sourceAlignment.representedPaths} paths/${sourceAlignment.representedSegments} segments · ` +
                        `pixel/mesh offsets [${sourceAlignment.offsets.map((entry) => `${entry.offset.toFixed(3)}:${entry.pixelHits}pixel/${entry.meshHits}mesh/${entry.both}both/${entry.pixelOnly}pixel-only/${entry.meshOnly}mesh-only/${entry.outsideImage}outside/${entry.samples}samples`).join(';')}] · ` +
                        `${sourceAlignment.reviews.length} discrepancy reviews (first 12) [${sourceAlignment.reviews.map((review) => `P${review.pathIndex}:S${review.segmentIndex}/${review.pixelOnly}pixel-only/${review.meshOnly}mesh-only`).join(';')}] · budget exhausted ${sourceAlignment.sampleBudgetExhausted} · ${sourceAlignment.limitation} · wall certified false · 0 edges admitted · 0 runs restored`
                    : 'G.5Z.42 source image alignment unavailable';
                cartographyAuditRuntimeWitness.dataset.cartographySamplingAlignment = alignment
                    ? `G.5Z.41 sampling alignment · ${alignment.scope} · ${alignment.representedPaths} paths/${alignment.representedSegments} segments · ` +
                        `paired offsets [${alignment.offsets.map((entry) =>
                            `${entry.offset.toFixed(3)}:${entry.positive}positive/${entry.negative}negative/${entry.paired}paired/${entry.positiveOnly}positive-only/${entry.negativeOnly}negative-only/${entry.samples}samples`).join(';')}] · ` +
                        `${alignment.lowInkReviews.length} low-ink reviews (first 12) [${alignment.lowInkReviews.map((review) =>
                            `P${review.pathIndex}:S${review.segmentIndex}/${review.directHits}direct/${review.bestHits}offset@${review.bestOffset.toFixed(3)}/${review.classification}`).join(';')}] · ` +
                        `budget exhausted ${alignment.sampleBudgetExhausted} · ${alignment.limitation} · wall certified false · 0 edges admitted · 0 runs restored`
                    : 'G.5Z.41 sampling alignment unavailable';
                cartographyAuditRuntimeWitness.dataset.cartographyIllustratedCorrespondence = correspondence
                    ? `G.5Z.40 illustrated boundary correspondence · ${correspondence.scope} · ` +
                        `${correspondence.representedPaths} paths/${correspondence.representedSegments} segments · ` +
                        `offset samples [${correspondence.offsets.map((entry) =>
                            `${entry.offset.toFixed(3)}:${entry.inkHits}/${entry.samples}`).join(';')}] · ` +
                        `${correspondence.lowCorrespondence.length} low-ink segment reviews (first 12) · ` +
                        `directional reviews ${correspondence.independentDirectionalReviewRuns} · ` +
                        `budget exhausted ${correspondence.sampleBudgetExhausted} · ${correspondence.limitation} · ` +
                        `wall certified false · 0 edges admitted · 0 runs restored`
                    : 'G.5Z.40 illustrated boundary correspondence unavailable';
                cartographyAuditRuntimeWitness.dataset.cartographyDirectionalInkContext =
                    `G.5Z.39 directional ink context · ${directionalContext.scope} · source ${directionalContext.sourcePresent ? 'present' : 'INCOMPLETE'} · ` +
                    `${directionalContext.reviewRuns} review runs · ` +
                    directionalContext.reviews.map((review) =>
                        `${review.axis}@(${review.start.join(',')})-(${review.end.join(',')})/${review.samples}samples/` +
                        `${review.nearRepresented}near/${review.parallelInkSamples}parallel/${review.perpendicularInkSamples}perpendicular/` +
                        `${review.branchingSamples}branches/local-overlap[${review.localReviewOverlaps.map((overlap) =>
                            `${overlap.localComponent}:${overlap.matchingSamples}`).join(',')}]/${review.classification}`).join(';') +
                    ` · ${directionalContext.limitation} · wall certified false · 0 edges admitted · 0 runs restored`;
                cartographyAuditRuntimeWitness.dataset.cartographyIndependentCoverage =
                    `G.5Z.38 independent wall coverage · ${independentWallCoverage.scope} · source ${independentWallCoverage.sourcePresent ? 'present' : 'INCOMPLETE'} · ` +
                    `${independentWallCoverage.meshCellsExamined} mesh cells examined · ${independentWallCoverage.runs.length} directional ink runs · ` +
                    `${independentWallCoverage.reviewRuns} review runs / ${independentWallCoverage.reviewSamples} unrepresented run samples (overlap possible) · ` +
                    `review bounds [${independentWallCoverage.reviewBounds.join(';')}] · ${independentWallCoverage.limitation} · ` +
                    `whole illustration certified false · missing walls certified false · wall certified false · 0 edges admitted · 0 runs restored`;
                // IV.30.1G.5Z.37 — Evidence ledger. A completed local review is
                // not a certificate for the illustration, nor is a quiet frontier
                // evidence of a wall. Fail closed if any source is unavailable.
                const ledgerSourcesPresent = Boolean(coverage && independentSurvey &&
                    disposition && frontier && termination && geometry &&
                    Array.isArray(endpointRecords));
                const evidenceLedger = {
                    diagnosticOnly: true, scope: 'bounded-cartography-evidence-not-whole-illustration',
                    sourcesPresent: ledgerSourcesPresent,
                    sampledBoundary: coverage ? {
                        total: coverage.sampledEdges, represented: coverage.representedSampledEdges,
                        quietNotWalls: coverage.quietUnrepresentedEdges,
                        corroboratedUnrepresentedReview: coverage.corroboratedUnrepresentedEdges,
                        unverified: coverage.unverifiedUnrepresentedEdges
                    } : null,
                    independentInk: independentSurvey ? {
                        examined: independentSurvey.meshCellsExamined,
                        candidateComponents: independentSurvey.componentCount,
                        reviewComponents: independentSurvey.reviewComponentCount
                    } : null,
                    localReviews: disposition ? {
                        complete: disposition.localReviewsComplete,
                        open: disposition.localReviewsOpen,
                        total: disposition.components.length
                    } : null,
                    openTerminations: Array.isArray(endpointRecords) ? endpointRecords.length : null,
                    quietSuppressedEdges: frontier?.classifiedEdges ?? null,
                    graphCycleOnly: geometry?.cycleCandidate === true &&
                        geometry?.geometryCertified !== true,
                    outstanding: [
                        'whole-illustration-wall-coverage-unverified',
                        'independent-ink-survey-is-candidate-only',
                        'quiet-frontier-is-not-wall-evidence',
                        'open-chain-terminations-not-wall-certified'
                    ],
                    wholeIllustrationCoverageCertified: false,
                    missingWallsCertified: false, wallCertified: false,
                    admittedEdges: 0, restoredRuns: 0
                };
                cartographyEvidenceAudit.cartographyEvidenceLedger = evidenceLedger;
                cartographyAuditRuntimeWitness.dataset.cartographyEvidenceLedger =
                    `G.5Z.37 evidence ledger · ${evidenceLedger.scope} · sources ${ledgerSourcesPresent ? 'present' : 'INCOMPLETE'} · ` +
                    `sampled ${evidenceLedger.sampledBoundary ? `${evidenceLedger.sampledBoundary.represented}/${evidenceLedger.sampledBoundary.total} represented, ${evidenceLedger.sampledBoundary.quietNotWalls} quiet-not-walls, ${evidenceLedger.sampledBoundary.corroboratedUnrepresentedReview} corroborated-review, ${evidenceLedger.sampledBoundary.unverified} unverified` : 'unavailable'} · ` +
                    `local reviews ${evidenceLedger.localReviews ? `${evidenceLedger.localReviews.complete}/${evidenceLedger.localReviews.total} complete, ${evidenceLedger.localReviews.open} open` : 'unavailable'} · ` +
                    `independent ink ${evidenceLedger.independentInk ? `${evidenceLedger.independentInk.examined} examined/${evidenceLedger.independentInk.candidateComponents} components/${evidenceLedger.independentInk.reviewComponents} review` : 'unavailable'} · ` +
                    `${evidenceLedger.openTerminations ?? 'unknown'} open terminations · ${evidenceLedger.quietSuppressedEdges ?? 'unknown'} suppressed frontier edges · ` +
                    `graph cycle only ${evidenceLedger.graphCycleOnly} · outstanding [${evidenceLedger.outstanding.join(',')}] · ` +
                    `whole illustration certified false · missing walls certified false · wall certified false · 0 edges admitted · 0 runs restored`;
