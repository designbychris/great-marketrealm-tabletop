...
final class TabletopShortcode
{
    ...
    public function render(
        ...
        if ($tableId === '') {
            ...
            $sceneManager = TableSceneManagerFactory::make();
            $campaigns = [];
            $atlasScenes = [];
            ...
            foreach ($registry->all() as $table) {
                ...
                if ($isOwner) {
                    foreach ($sceneManager->scenes($table->id()) as $atlasScene) {
                        if ($atlasScene->isGeneratedSurface() || $atlasScene->mapAttachmentId() < 1) {
                            continue;
                        }
                        $atlasScenes[] = [
                            'table_id' => $table->id(),
                            'table_name' => $table->name(),
                            'scene_id' => $atlasScene->id(),
                            'scene_name' => $atlasScene->name(),
                        ];
                    }
                }
