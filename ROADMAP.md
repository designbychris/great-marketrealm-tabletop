...
- [x] **IV.30.1G — The Cartographer Looks Again / Adaptive Evidence Model** — replaces fixed-darkness dependence in imported-map Structural tracing with a local-contrast evidence layer built from summed-area luminance statistics; pale walls can be recognised relative to nearby floor, dark floor is not automatically promoted to wall ink, and the existing hatch/side-density safeguards plus Keeper review remain authoritative.
- [x] **IV.30.1G.1 — Corners, Junctions & Wall Bodies** — layers architectural topology over adaptive Structural traces: connected corners, T/X junctions and collinear continuation increase confidence, thick wall-body profiles add supporting evidence, while truly isolated strokes are conservatively demoted without being discarded.
- [x] **IV.30.1G.2 — Doorway & Threshold Reasoning** — promotes one-grid wall gaps to doorway candidates only when supported wall runs bracket the opening, the gap is genuinely clearer than the wall ink, plausible floor exists on both sides, and the resulting threshold remains a review-first suggestion with forensic evidence rather than an automatic door.
- [x] **IV.30.1G.3 — Noise, Furniture & Annotation Rejection** — adds coarse connected-component and local clutter evidence so short compact annotations, furniture-like outlines and isotropic hatch/rubble clusters are demoted before threshold reasoning; only isolated low-confidence marks with multiple noise signals are rejected outright, while connected architecture is explicitly defended.
- [x] **IV.30.1G.4 — Partial Contour Recovery** — replaces Living Contour’s all-or-nothing review failure with ranked safe contour sections: closed boundaries remain full-confidence paths, useful open chains are retained as lower-confidence partial contours with unresolved ends explicitly preserved, and Hybrid Judgement may consume those organic sections without inventing bridges across missing evidence.
- [x] **IV.30.1G.4A — Hybrid Partial-Contour Preservation** — lets Hybrid Judgement fall back to standalone certified Living Contour evidence when connected-floor analysis proves less, ranks organic paths instead of failing at the review cap, and preserves partial contour unresolved ends without automatic bridging.
- [x] **IV.30.1G.4B — Gap & Threshold Classification** — reuses G.2 structural doorway evidence inside Living Contour and Hybrid, splits certified doorway/passage spans out of organic contours instead of sealing them, classifies unresolved ends as doorway/passage, noise gap, map edge or uncertain boundary, and keeps every protected opening review-first and unbridged.
- [x] **IV.30.1G.5 — Boundary-Side & Wall-Band Reasoning** — distinguishes dominant border-connected exterior whitespace from playable floor, requires sustained playable-floor depth on the inward side of a candidate contour, and prevents Living Contour/Hybrid from hopping through cave crosshatching to the far side of an illustrated wall band.
- [x] **IV.30.1G.5A — Semantic Boundary Classification** — Classify Living Contour evidence as structural wall, terrain, obstacle, decoration/noise or uncertain before automatic LOS drafting; protected thresholds remain authoritative.
- [x] **IV.30.1G.5B — Structural Evidence Corroboration** — Hybrid requires Living Contour agreement or asymmetric wall-body ink before grid-aligned structural evidence can become LOS geometry.
- [x] **IV.30.1G.5C — Semantic Continuity & Region Topology** — Hybrid preserves locally weak spans when they belong to a coherent connected playable boundary, while thresholds remain explicit topological breaks.
- [x] **IV.30.1G.5D — Contour Chain Certification** — Hybrid certifies coherent Living Contour chains as region-scale evidence, considers both connected and standalone Living readers, preserves weak spans inside certified boundaries, and keeps thresholds and tiny interior marks protected.
- [x] **IV.30.1G.5E — Boundary Role & Interior Feature Classification** — Living Contour distinguishes playable-region perimeters from hills, rubble, terrain, compact interior features and decorative marks before granting LOS-wall authority; Hybrid chain certification cannot promote classified interior features.
- [x] **IV.30.1G.5F — Playable-Space Adjacency & Threshold Connectivity** — Living Contour now validates boundary roles against playable-floor adjacency, demotes enclosed terrain embedded in connected floor, and preserves short floor-continuous gaps as inferred thresholds even when no conventional door glyph was recognised.
- [x] **IV.30.1G.5G — Portal Pairing & Boundary-Side Reasoning** — Living Contour pairs unresolved boundary ends, checks jamb/tangent compatibility, samples playable floor through the break and reasons about both boundary sides so genuine portals stay open while tiny structural breaks can be distinguished from thresholds.
- [x] **IV.30.1G.5H — Boundary Graph Reconstruction & Evidence Bridging** — Living Contour reconstructs a graph across fragmented structural chains and permits only short, tangent-compatible, boundary-side-certified evidence bridges; portal and threshold-connected ends remain explicit open breaks.
- [x] **IV.30.1G.5I — Playable Region Closure & Perimeter Inference** — Living Contour can reason outward from certified playable regions, locally recover floor obscured by illustration, and infer only ink-corroborated playable/non-playable perimeter transitions while exterior whitespace and protected thresholds remain vetoes.
- [x] **IV.30.1G.5J — Playable Surface Reconstruction & Occlusion Recovery** — Reconstruct short, corroborated spans of playable floor hidden beneath dense illustration before semantic boundary reasoning; visible playable shores and lateral floor support are required, while sustained structural wall ink remains a veto.
  - [x] **IV.30.1G.5J.1 — Monotonic Recovery Correction** — G.5J occlusion evidence is additive-only: preserve the pre-recovery G.5I contour baseline, fall back to it on uncertainty, and merge only genuinely new recovery evidence within the existing review budget.
- [x] **IV.30.1G.5K — Interior Occupancy & Illustrated Floor Reasoning** — Treat compact enclosed illustration as possible occupancy over playable floor when three-direction enclosure, opposing-floor support, and local quiet-floor evidence agree; sustained wall bands veto inference, and all inferred surface remains additive-only.
- [x] **IV.30.1G.5L — Reconstructed Surface Propagation & Contour Re-certification** — Propagate G.5J/G.5K recovered playable-surface provenance into G.5I region closure, locally re-certify corroborated outer perimeter transitions, preserve portals and structural-wall vetoes, and retain the G.5J.1 additive-only contour contract.
...
- [x] **IV.32.4B — The Veil Goes 16-Bit** — restyles the existing Living Veil as near-black pixel-dithered unknown space, faded 16-bit exploration memory, stepped live-sight and memory frontiers, plus a Keeper-only Player Fog preview frame; Fog/LOS authority and geometry remain unchanged.
- [x] **IV.32.4C — The Keeper Lights the Pixels** — gives the existing Torch, Candle, Lantern, Brazier and Magical Light distinct CSS-drawn 16-bit battlefield markers and restrained stepped glow personalities, with doused/reduced-motion states preserved and no change to lighting, Fog or LOS authority.
- [x] **IV.32.4D — The Battlefield's Final Inspection** — closes the battlefield pixel pass with one explicit visual stack for map/grid, footsteps, Vision/lights, Living Veil, Keeper thresholds, miniatures and targeting feedback; decorative overlays remain pointer-transparent and all geometry/authority stays unchanged.
...
- [x] IV.35.3B — select, move, rotate, scale, duplicate and remove Scene Objects. **SERVER + BROWSER CERTIFIED.**
...
G.5Z.21 adds a second, separately audited continuity-restoration class without relaxing the certified G.5Z.18 micro-gap rule. Suppressed open-paper runs may be restored only when they remain exact same-source perimeter topology, are both-bounded and non-branching, contain 5–8 original perimeter edges, and their G.5Z.20 perimeter travel is no more than 0.3 contour cells at the published one-decimal audit precision. Runs of 9+ edges remain suppressed by construction. Restoration reuses only the original perimeter edges: no endpoint search, nearest matching, snapping, interpolation, synthetic bridge, weakened structural/threshold veto, or change to the 200 review-object ceiling or 256-vertex cap is introduced. The Evidence Audit separately reports eligible short runs, eligible edges, and restored edges; diagnostic marks remain never-save.
