...
final class WeaponHandsAjaxController
{
    ...
    public function roll(): void
    {
        ...
        try {
            ...
            $action = sanitize_key((string) ($_POST['weapon_action'] ?? ''));
            $attackId = sanitize_key((string) ($_POST['attack_id'] ?? ''));
            $userId = get_current_user_id();
            $member = $this->members->find($tableId, $userId);

            if ($member === null || $member->status() !== TableMemberStatus::ACTIVE) {
                throw new \RuntimeException('Only an active Table member may reach for a weapon.');
            }

            $characterId = $member->companionCharacterId();
            if ($characterId === null) {
                throw new \RuntimeException('Bring a Companion Character to the Table before drawing a weapon.');
            }
