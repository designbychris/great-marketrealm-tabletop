...
{
    /**
     * Keeper light presets. Bright/dim distances are measured in feet.
     *
     * Phase IV.31.1 keeps the familiar 5e-style bright + equal dim band while
     * giving each source its agreed bright-light identity on the Lantern Rack.
     *
    ...
    public function tend(): void {
        ...
        try {
            ...
            if(in_array($action,['light','douse','toggle'],true)){ $lit=$action==='light'?true:($action==='douse'?false:!$light->lit());$light=$light->withLit($lit);$this->lights->save($light);wp_send_json_success(['message'=>$light->label().($light->lit()?' lit.':' extinguished.'),'light'=>$light->toArray()]); }
