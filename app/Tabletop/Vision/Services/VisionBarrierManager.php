...
final class VisionBarrierManager
{
    ...
    public function add(string $tableId,int $userId,string $type,int $x1,int $y1,int $x2,int $y2):VisionBarrier{$scene=$this->guard($tableId,$userId);$barrier=new VisionBarrier('vision-'.bin2hex(random_bytes(6)),$scene->id(),$type,$x1,$y1,$x2,$y2,false);$this->barriers->save($tableId,$barrier);$this->refreshExploration($tableId,$scene);return $barrier;}
    public function toggleDoor(string $tableId,int $userId,string $id):VisionBarrier{$scene=$this->guard($tableId,$userId);foreach($this->barriers->forScene($tableId,$scene->id()) as $barrier){if($barrier->id()===$id){$barrier->toggleDoor();$this->barriers->save($tableId,$barrier);$this->refreshExploration($tableId,$scene);return $barrier;}}throw new RuntimeException('That vision door could not be found.');}
    public function remove(string $tableId,int $userId,string $id):void{$scene=$this->guard($tableId,$userId);$this->barriers->delete($tableId,$scene->id(),$id);$this->refreshExploration($tableId,$scene);}
    ...
    private function guard(string $tableId,int $userId):TableScene{$member=$this->members->find($tableId,$userId);if($member===null||$member->status()!==TableMemberStatus::ACTIVE||$member->role()!==TableMemberRole::DUNGEON_MASTER){throw new RuntimeException('Only the Dungeon Master may alter the vision layer.');}foreach($this->scenes->forTable($tableId) as $scene){if($scene->isActive()){return $scene;}}throw new RuntimeException('Open a Scene before altering the vision layer.');}
}
